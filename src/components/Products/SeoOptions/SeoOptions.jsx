        ...
        <section className="seoOptionsWrapper">
            <div className="seoOptionsInputs">
                {formState?.seotags.map((item, index) =>
                    <div key={index} className={`seoOptionsInput flexAlignCenter ${index === 0 ? "" : "marginTop20"} `}>
                        <input required onChange={(e) => handleChange(e, index)} type="text" placeholder="Enter Single Tag" />
                        {formState.seotags.length > 1 && index !== formState.seotags.length - 1 && (
                            <CloseIcon onClick={() => handleRemove(index)} className="closeIcon" />
                        )}
