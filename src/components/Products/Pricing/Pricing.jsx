        ...
        <section className="pricingWrapper">
            <div className="pricingInputs flexJustifyStart flexColumn">
                ...
                <div className="pricingInputs marginBottom0 marginTop20">
                    <div className="onSaleCheckbox">
                        <Switch
                            disabled={
                                formState?.price === null ? true :
                                    formState?.price === "" ? true :
                                        formState?.price === 0 ? true : false}
                            onChange={handleChange}
                            {...label}
                            required
                            name='onsale'
                            defaultChecked={formState?.onsale} />
                        <label className='onSaleInfoIcon'>On Sale <Tooltip className='flex' title="Please Enter Price First" placement="right"> <InfoIcon /></Tooltip> </label>
                    </div>
                </div>
            </div>
            {
                formState?.onsale === 1 && <div className='pricingInputs'>
                    <div className="pricingInputBox">
                        ...
                        <div className="pricingInput paddingEnd0">
                            <input
                                ...
                                type="number"
                                min={0}
                                max={formState?.discount_type === "percentage" ? 99 : (formState?.price - 1)}
                                name="discount"
                                required
                                placeholder="Enter Discount"
                                onInput={(e) => {
                                    const value = parseInt(e.target.value);
                                    if (value > (formState?.discount_type === "percentage" ? 99 : formState?.price)) {
                                        e.target.value = formState?.discount_type === "percentage" ? 99 : (formState?.price - 1);
                                    }
                                }} />
                            <div className='discountIcon'>
                                <span onClick={() => handleDiscountType("percentage")} className={`${formState?.discount_type === "percentage" && "discountIconActive"}`}>%</span>
                                <span onClick={() => handleDiscountType("amount")} className={`${formState?.discount_type === "amount" && "discountIconActive"}`}>₹</span></div>
                        </div>
                    </div>
                    <div className="pricingInputBox">
                        ...
                        <div className="pricingInput">
                            <span>₹</span>
                            <input disabled type="text" required placeholder="Enter Price" name="price" value={formState?.salesprice} />
                        </div>
                    </div>
                </div>
            }
            <div className="pricingInputs flexColumn width50 gap0">
                ...
                <div>
                    ...
                    {formState?.instock === 1 && <div className="pricingInputBox" style={{ width: '40%' }}>
